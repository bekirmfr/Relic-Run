...
1. `Tools/capture` exports N seeded runs from the JS build (both modes, sandbox flight recorder).
2. `CorpusDiffer` loads each run, feeds the same seed and loadout into `CombatEngine`, and diffs the
   event stream field-by-field: type, amount, relic, copy index, depth, tick, and the full snapshot.
3. Any divergence prints the first mismatching event with both sides' context.
